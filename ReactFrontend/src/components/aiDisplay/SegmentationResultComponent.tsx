...
interface PredictResult {
  ai_type: string;
  prediction: { detections: SegmentationDetection[] | "No detections"; label: "positive" | "negative" };
}
...
const SegmentationResultComponent: React.FC<DemoPredictResultProps> = ({
  ...
  useEffect(() => {
    ...
    image.onload = () => {
      ...
      if (showSegments && Array.isArray(predictResult.prediction.detections)) {
        ...
        <div className="w-[50%] border rounded-[5px] p-10" >
          ...
          <div className="flex justify-start mb-2 mt-4">
            {aiDataProp.ai_tag.map((tag: string, index: number) => (
              <span key={index} className="w-fit bg-indigo-600 rounded-[5px] me-2 px-2.5 py-0.5 text-white text-lg font-normal">
                {tag}
              </span>
            ))}
          </div>
          ...
          <div className="py-4">
            ...
            <table className="min-w-full mt-4 bg-white rounded-lg shadow">
              ...
              <tbody>
                {Array.isArray(predictResult.prediction.detections) ? (
                  predictResult.prediction.detections.map((detection, index) => (
                    ...
                    <tr key={index} className="bg-gray-100 border-b">
                      ...
                      <td className="py-3 px-4 text-gray-800 text-xl">
                        {/* <span className="py-3 px-4 text-indigo-600 text-xl font-medium"></span> */}
                        {` ${detection.label}`}
                      </td>
                    </tr>
                  ))
                  ...
                  <tr className="bg-gray-100 border-b">
                    <td colSpan={2} className="text-center py-4">
                      {predictResult.prediction.detections === "No detections"
                        ? "ไม่มีการตรวจพบ"
                        : "ไม่มีข้อมูล"}
                    </td>
                  </tr>
