...
type SubDocListProps = {
  docsId: string;
  subDocuments: SubDoc[];
  onSubDocClick: (index: number, subIndex: number) => void;
  // onShowSubEditOptionModal: (
  //   event: React.MouseEvent<HTMLSpanElement, MouseEvent>,
  //   index: number,
  //   subIndex: number
  // ) => void;
  onChangeSubTitle:any,
  patchSubDocsTitle:any,
...
const SubDocList: React.FC<SubDocListProps> = ({
  ...
  const [renameSubDocId, setRenameSubDocId] = React.useState(null);
  ...
  const handleClose = (subDocsId: string) => {
   ...
   return (
    <>
      {subDocuments.map((subDoc, subIndex) => {
        ...
        return (
          <div className=" flex w-full pl-6 justify-between " key={subDoc.subDocsId}>
            ...
            <div className="mx-2 flex items-center h-full w-fit">
              ...
              >
                <i className="bi bi-three-dots  text-gray-600 justify-between hover:bg-gray-100 rounded-lg gap-3 cursor-pointer"
